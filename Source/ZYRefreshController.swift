...
open class ZYRefreshController: UIViewController {
    
    weak open var scrollView : UIScrollView?
    weak open var dataSource : ZYRefreshDataSource? = nil
    
    private var reachable: Bool = ZYRefresh.reachable()
    ...
    /**
     ...
     @param option 刷新空间样式
     */
    open func setupRefresh(scrollView: UIScrollView,options: ZYRefreshOption){
        self.scrollView = scrollView
        if options.rawValue == ZYRefreshOption.none.rawValue {
            if self.responds(to: #selector(headerRefreshing)) {
                self.headerRefreshing()
            }
        }
        if options.rawValue & ZYRefreshOption.header.rawValue == 1 {
            ...
            if options.rawValue & ZYRefreshOption.autoHeader.rawValue == 4 {
                self.headerRefreshing()
            }
            scrollView.mj_header = header
        }
        if options.rawValue & ZYRefreshOption.footer.rawValue == 2 {
            ...
            if options.rawValue & ZYRefreshOption.autoFooter.rawValue == 8 {
                if self.currentPage == 0 {
                    self.refreshing = true
                }
                self.footerRefreshing()
            }else if options.rawValue & ZYRefreshOption.defaultHidden.rawValue == 16 {
...
extension ZYRefreshController :DZNEmptyDataSetSource,DZNEmptyDataSetDelegate{
